...
read_table("https://www.o3d.org/npgo/data/NPGO.txt")
...
sstdat<-get_ersst_v5_new(years=c(min(Yrlist$year):max(Yrlist$year)),
                           ...
                           ncfilename="sst.mnmean_1_23_2025_1.nc",
                           latrange=c(44,50),
                           lonrange=c(-125,-120)
)%>%
  filter(!is.na(sst))%>%
...
## rolling_year_window<-15
rp<-SalmonForecasting::rolling_perf(results3,dat,rolling_year_window,num_models,TY_ensemble,model_list,alpha=alpha)
# rp$performance
