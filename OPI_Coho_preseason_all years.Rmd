...
## rolling_year_window<-15
rp<-rolling_perf(results,dat,rolling_year_window,3,yr_end=yr_end)
# rp$performance
...
Figure3<-ggplot((results_old$forecasts%>%mutate(model_name="Current_OPI") %>%
                   bind_rows(results_best ) %>%
                   mutate(Model=sub("_"," ",model_name)) %>%
                   mutate(Model=ifelse(grepl(" \\+ ",Model),"Best Individual",Model))%>%
                   mutate(pct_error=log10(((predicted_abundance-abundance)/abundance)+1))) %>%
                  filter(year<yr_end),
                aes(x=year,y=pct_error,color=Model))+
  geom_hline(yintercept=0)+
  geom_line(lwd=1.25)+
  scale_color_manual(values=c("darkblue","darkred"))+
  xlab("")+
  ylab(expression(paste(log[10], "(% error +1)")))
