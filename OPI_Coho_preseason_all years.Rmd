...
save(results3,file="results3/forecasts.rds")
}else{
...
## rolling_year_window<-15
rp<-SalmonForecasting::rolling_perf(results3,dat,rolling_year_window,num_models,TY_ensemble,model_list)
# rp$performance



# num_models<-10
ens<-SalmonForecasting::ensemble(rp$all_mods %<>% group_by(year) %>% mutate(rank=rank(MAPE)) %>% ungroup,dat,TY_ensemble,k,slide=15,num_models,stack_metric,stretch=FALSE,do_stacking = FALSE)
