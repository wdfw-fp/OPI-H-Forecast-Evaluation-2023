---
title: "OPI Forecasts"
author: Thomas Buehrens, WDFW (tbuehrens@dfw.wa.gov), Mark Sorel, WDFW (mark.sorel@dfw.wa.gov), Cassandraw Leeman, ODFW (cassandra.r.leeman@odfw.oregon.gov), and Shannon Conley, WDFW (shannon.conley@dfw.wa.gov)
output:
  html_document:
    code_folding: hide
    fig_caption: yes
...
#===========================
date_start_analysis<-ymd("2000/1/1") #this will be the first date of data used in the analysis (and this month/day first in years thereafter)
...
## Get Raw Data
Here we load the response variable (observed OPI coho abundance) and covariates used in past forecasts from Erik Suring's GitHub repository [**OPIH Evaluation**](https://github.com/ErikSuring/OPIH_Evaluation). The version of the response variable (observed OPI coho abundance) in Erik's GitHub page differs from the official data set used by ODFW and WDFW, so consequently, we also load that official data set provided by Cassandra Leeman (ODFW) on 9/8/2023 and we use those numbers instead of those in Erik's repository for years where they differ. Finally, we also load some auxiliary covariates including:
...
dat<-OPIHData%>%
  ...
  filter(year>=yr_start)#%>%mutate(across(!c(year,species,period,abundance),boxcox_scale(.)))
...
Figure2<-ggplot(results$forecasts,aes(x=year,y=predicted_abundance,col=model_name))+
  geom_line()+
  geom_line(data=results_best,mapping=aes(x=year,y=predicted_abundance),size=1.5,col="black")+
...
Figure 2 shows the annual one-step-ahead predictions of abundance (lines) for the entire top model set (based on in-sample AIC) that was used in one step ahead evaluation and ensemble development relative to the observed abundance (black dots).
...
[DONE-ish] Multivariate ENSO Index (Oct-Dec; t-1)
[DONE-ish] Spring Transition (Julian date; t-1)
