pacman::p_load(tidyverse,reshape2,dataRetrieval,lubridate,parallel,foreach,doParallel,rnoaa,MARSS,buoydata)
source(here::here("functions/get_ersst_v5_new.r"))
source(here::here("functions/getbuoydata26.r"))
...
if(!file.exists(here::here("data/buoy_data.csv"))){
  buoy_data <- getbuoydata26(buoylist = buoylist,years=years)
...
SST1<-get_ersst_v5_new(years=c(2010,2026),
...
write.csv(results%>%
          ...
          here::here("data/January_ERSST_v5_2026.csv"),row.names = F
          )
