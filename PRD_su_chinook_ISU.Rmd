---
title: Columbia Summer Chinook Inseason Forecast for Counts 6/7-8/20 @ Priest Rapids Dam
...
#===========================
date_start_analysis<-ymd("2000/6/7") #this will be the first date of data used in the analysis (and this month/day first in years thereafter)
date_end_analysis<-ymd("2022/8/20") #this will be the last date of data used in the analysis (and this month/day last in years preceding)
...
forecast_period_start_d<-7 #this will be the month day associated with the first month/day of the seasonal estimate period each year...can be after first data used to estimate it or the same
use_freshest_data = T #use all data up to "today" or only up to beginning of forecast period
covariates<-c("lag1_NPGO","lag2_NPGO","lag1_PDO","lag2_PDO","log_BON_CK","lag1_log_jCK","lag2_PC1","zl_flow")
#==================
#forecasting params
#==================
leave_yrs<- 12
covariates<-c("lag1_NPGO","lag2_NPGO","lag1_PDO","lag2_PDO","lag1_log_jCK","log_BON_CK","lag2_PC1","zl_flow") 
...
dat<-get_dam_data(
  ...
  dam = dam,
  species = species
)%>%filter(date < as.Date("2022-07-11"))
...
# Jacks
#======
jCK<-read_csv("http://www.cbr.washington.edu/dart/cs/php/rpt/adult_annual.php?sc=1&outputFormat=csv&proj=PRD&startdate=6%2F7&enddate=8%2F20")%>%
  arrange(Year)%>%
...
# BON_CK
#======
BON_CK<-read_csv(paste0("http://www.cbr.washington.edu/dart/cs/php/rpt/adult_annual.php?sc=1&outputFormat=csv&proj=PRD&startdate=6%2F1&enddate=",month(last(dat$date)),"%2F",day(last(dat$date))))%>%
  arrange(Year)%>%
  dplyr::select(year=Year, `Chinook`)%>%
  mutate(year = year, log_BON_CK = log(ifelse(`Chinook`==0,NA,`Chinook` + 1)))%>%
  filter(!is.na(year) & !is.na(log_BON_CK))%>%
  dplyr::select(year,log_BON_CK)
#=========================================================
...
#======
# 
# BON_CK<-read_csv(paste0("http://www.cbr.washington.edu/dart/cs/php/rpt/adult_annual.php?sc=1&outputFormat=csv&proj=BON&startdate=6%2F1&enddate=8%2F20"))%>%
#   arrange(Year)%>%
#   dplyr::select(year=Year, `Chinook`)%>%
#   mutate(year = year+1, lag1_log_BON_CK = log(ifelse(`Chinook`==0,NA,`Chinook`)))%>%
#   filter(!is.na(year) & !is.na(lag1_log_BON_CK))%>%
#   dplyr::select(year,lag1_log_BON_CK)
...
if(find_best ==T){
  ...
  saveRDS(best_covariates,"best_covariates_chinook_prd.rds")
}
...
best_covariates<-readRDS("best_covariates_chinook_prd.rds")
