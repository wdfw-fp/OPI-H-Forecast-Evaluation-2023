---
title: Baker River Sockeye Inseason Forecast for Counts 6/1-12/31 @ Lower Baker Dam
...
last_data<-as.Date("2021-07-20")
#==================
#forecasting params
#==================
leave_yrs<- 11
covariates<-c("lag2_NPGO","lag1_log_SAR2","lag2_log_smolts","var_flow", "lag1_PDO","lag1_NPGO","lag2_PDO","pink_ind","lag1_log_SAR1")
p1_covariates_only=c("var_flow")
plot_results = F
...
results<-inseason_forecast(series$series,
                  leave_yrs=leave_yrs,
                  covariates= c("lag2_NPGO","lag1_log_SAR2","lag2_log_smolts","var_flow"), #use to automate variable selection: best_covariates[[1]][[best_covariates[[2]]$model_num[1]]],
...
psf_performance <- read_csv("https://raw.githubusercontent.com/casruff/Salmon_Forecast_Example/test/sockeye_preseason_fcst_performance.csv")%>%
  ...
  rename(year = Year,predicted_abundance = Estimate)
...
performance_comp %>%
kbl(caption = paste0("Table 3. Comparison of forecast performance of ISU using catch and Baker dam counts versus stack weighted pre-season forecast for years 2012 - 2020."),digits =3)%>%
  kable_classic(full_width = F, html_font = "Cambria")
...
p<-ggplot(results,aes(x=year,y=predicted_abundance))+ 
  ...
  geom_line()+
  geom_line(data = psf_performance, color = "orange")+ 
