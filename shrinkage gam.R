...
mod<-mgcv::gam(log(abundance)~
              ...
              te(lag1_log_JackOPI,year)#+ #this puts at L2 penalty on the coefficient
...
#obs vs fitted
plot(abundance~year,data=dat[(54-leave):54,],type="p",pch=19,col="black",ylim=c(0,2000))
points(seq(to=2023,length.out=leave+1),out,type="o",col="red",pch=19)

#MAPE
mean((abs(dat$abundance[(54-leave):54]-out)/dat$abundance[(54-leave):54]),na.rm=T)
#RMSE
sqrt(mean(((dat$abundance[(54-leave):54]-out)^2),na.rm=T))
