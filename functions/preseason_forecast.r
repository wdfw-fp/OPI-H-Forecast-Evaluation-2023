...
preseason_forecast<-function(series,
                            leave_yrs,
                            covariates,
  ...
  tdat2<-forecasts%>%
    ...
    left_join(forecasts%>%dplyr::select(model,year,abundance,species,all_of(unique(unlist(covariates))))%>%filter(model==as.character(c))%>%dplyr::select(!model))%>%
    mutate(error = predicted_abundance-abundance,
           pct_error=scales::percent(error/abundance),
           model = "ensemble"
    )
