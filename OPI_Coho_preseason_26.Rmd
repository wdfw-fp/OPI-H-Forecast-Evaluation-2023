...
date_end_analysis<-ymd("2025/12/31") #this will be the last date of data used in the analysis (and this month/day last in years preceding)
...
alpha<-0.25 # exponential smoothing of APE calculations
...
sstdat<-get_ersst_v5_new(years=c(min(Yrlist$year):max(Yrlist$year)),
                           ...
                           ncfilename="sst.mnmean_1_5_2026.nc",
                           latrange=c(44,50),
                           lonrange=c(-125,-120)
)%>%
  filter(!is.na(sst))%>%
  ...
  # bind_rows(read_csv(here::here("data/January_ERSST_v5_2025.csv"))%>%
...
dat%>%
  dplyr::select(-c(year,species,period,abundance,`...1`))%>%
  cor()%>%
...
if(!file.exists("results/forecasts3.rds")|redo_forecasts==TRUE){
...
save(results3,file="results/forecasts3.rds")
}else{
  load("results/forecasts3.rds")
}
...
SalmonForecasting::check_alpha(
  One_ahead_out=results3,dat=dat,
num_models=num_models,TY_ensemble=TY_ensemble,
screen_metric="MAPE",
  model_list=model_list,
...
## rolling_year_window<-15
rp<-SalmonForecasting::rolling_perf(results3,dat,rolling_year_window,screen_metric=MAPE,num_models,TY_ensemble,model_list,alpha=alpha)
# rp$performance
...
top_mods<-one_step_ahead(series=dat |> mutate(across(all_of(covariates),\(x)c(scale(x)))),
                  ...
                  TY_ensemble=1,
                  # covariates= best_covariates[[1]][best_covariates[[2]]$model_num[1:num_models]],
                  best_covariates[[1]][rp$top_mods |> filter(year==2025) |> pull(model) |> as.numeric()],
                  first_forecast_period = first_forecast_period,
                  plot_results = plot_results,
                  write_model_summaries = write_model_summaries,
                  forecast_period_start_m =  forecast_period_start_m, #inclusive 
...
top_mod_coefs<-bind_rows(lapply(top_mods$mod,\(x)x[[1]]))

top_mod_ave_coef<-top_mod_coefs |> mutate(across(everything(),\(x)replace_na(x,0))) |>colMeans() |> tibble::enframe()

sum(ens$final_model_weights$MAPE_weight*top_mod_coefs$lag1_PDO)

top_mods_24<-one_step_ahead(series=dat |> mutate(across(all_of(covariates),\(x)c(scale(x)))),
                  leave_yrs=1,
                  TY_ensemble=1,
                  # covariates= best_covariates[[1]][best_covariates[[2]]$model_num[1:num_models]],
                  best_covariates[[1]][rp$top_mods |> filter(year==2024) |> pull(model) |> as.numeric()],
                  first_forecast_period = first_forecast_period,
                  plot_results = plot_results,
                  write_model_summaries = write_model_summaries,
                  forecast_period_start_m =  forecast_period_start_m, #inclusive 
...
top_mod_coefs_24<-bind_rows(lapply(top_mods_24$mod,\(x)x[[1]]))

top_mod_ave_coef_24<-top_mod_coefs_24 |> mutate(across(everything(),\(x)replace_na(x,0))) |>colMeans() |> tibble::enframe()
...
Figure 4 shows the average effect of each of the predictors on the forecast. There is also an intercept, which averaged `r round(top_mod_ave_coef |> filter(name=="intercept") |> pull(value),2)` and each of the top models had a lag 1 moving average (MA1) coefficient (average = `r round(top_mod_ave_coef |> filter(name=="ma1") |> pull(value),2)`), such that when  a positive forecast error (actual value > predicted) lead to a decrease in the next forecast.

```{r message=FALSE, warning=FALSE, results="show", fig.align='center',fig.show=TRUE, fig.cap="Figure 4. Effects of covariates on 2025 forecast."}


top_mod_ave_coef |> filter(!name%in%c("intercept","ma1","ar1","sar1")) |>
  left_join(
   dat |> mutate(across(all_of(covariates),\(x)c(scale(x))))|> filter(year==2025) |> dplyr::select(all_of(covariates) ) |> c() |> unlist() |> enframe() |> rename("coef"="value")
    
    
  ) |> mutate(loading=value*coef) |> 
  
  
  ggplot2::ggplot(aes(x=name,y=loading))+ggplot2::geom_col() +ggplot2::theme(axis.text.x = element_text(angle = 45, vjust = .9, hjust=1))+ylab("Average effect of predictor on forecast")

```

Figure 5 shows the average effect of each of the predictors on the 2024 forecast. There is also an intercept, which averaged `r round(top_mod_ave_coef |> filter(name=="intercept") |> pull(value),2)` and each of the top models had a lag 1 moving average (MA1) coefficient (average = `r round(top_mod_ave_coef |> filter(name=="ma1") |> pull(value),2)`), such that when  a positive forecast error (actual value > predicted) lead to a decrease in the next forecast.

```{r message=FALSE, warning=FALSE, results="show", fig.align='center',fig.show=TRUE, fig.cap="Figure 5. Effects of covariates on 2024 forecast."}


top_mod_ave_coef_24 |> filter(!name%in%c("intercept","ma1","ar1","sar1")) |>
  left_join(
    dat |> mutate(across(all_of(covariates),\(x)c(scale(x))))|> filter(year==2024) |> dplyr::select(all_of(covariates) ) |> c() |> unlist() |> enframe() |> rename("coef"="value")
...
Figure 6 shows time series of the z-scored covariate values.
...
dat %>% dplyr::select(all_of(covariates),year) %>% mutate(across(all_of(covariates),function(x)c(scale(x))),year=as.integer(year)) %>% pivot_longer(all_of(covariates),values_to = "Z-scored value") %>% filter(between(year,2000,2025)) %>% ggplot(aes(x=year,y=`Z-scored value`))+geom_point()+geom_line() + facet_wrap(~name)+geom_hline(yintercept = 0,lty=2)+scale_x_continuous(breaks=integer_breaks(),guide = guide_axis(check.overlap = TRUE))
